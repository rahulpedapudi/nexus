...
def rename_conversation(
    ...
    updated = conversation_service.rename_conversation(conv_id, body.title, db, current_user)
    if not updated:
...
def delete_conversation(
    ...
    success = conversation_service.delete_conversation(conv_id, db, current_user)
    if not success:
        raise HTTPException(status_code=404, detail="Conversation not found")
